  ...
  const handleClick = (data: DataProps) => {
    ...
    switch (data && data.action) {
      ...
      case 'copy':
        ...
        break;
      case 'copyurl':
        setClipboard(data.photourl || '');
        break;
      case 'copylink':
        setClipboard(data.mixtape || '');
        break;
      case 'custom':
  ...
  return (
    <>
      ...
      >
        ...
        {item && item.metadata?.photourl && (
          <MenuItem onClick={() => handleClick({ action: 'copyurl', photourl: item.metadata?.photourl })}>
            {Locale.ui_copyurl}
          </MenuItem>
        )}
        {item && item.metadata?.mixtape && (
          <MenuItem onClick={() => handleClick({ action: 'copylink', mixtape: item.metadata?.mixtape })}>
            {Locale.ui_copylink}
          </MenuItem>
        )}
